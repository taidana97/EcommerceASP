...
namespace BuildCompleteEcommerceWithASPNETCoreMVC.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var featuredProducts = db.Products.OrderByDescending(p => p.Id)
                .Where(p => p.Status && p.Featured).ToList();

            ViewBag.FeaturedProducts = featuredProducts;
            ViewBag.CountFeaturedProducts = featuredProducts.Count;
            ViewBag.LatestProducts = db.Products.OrderByDescending(p => p.Id)
